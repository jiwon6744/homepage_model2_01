				...
				<c:otherwise>
					<c:forEach items="${list }" var="dto">
						<tr>
							...
							<td class="table-bordered td"><c:forEach begin="1"
									end="${dto.indent }">
									<c:out value="&nbsp;&nbsp;" escapeXml="false"></c:out>
								</c:forEach> <c:if test="${dto.indent > 0 }">
									<img
										src="${pageContext.request.contextPath }/images/reply-back.png"
										style="width: 25px">
								</c:if> <a href="javascript:read('${dto.no }')"> ${dto.title }</a> <c:if
									test="${util:newImg(fn:substring(dto.wdate,0,10)) }">
									<img
										src="${pageContext.request.contextPath }/views/imgbbs/images/new.gif">
								</c:if></td>
							...
							<c:otherwise>
									...
									<img src="${pageContext.request.contextPath }/views/imgbbs/storage/${dto.filename}"
										style="width: 250px">
									</a>
								</c:otherwise>
							</c:choose>
